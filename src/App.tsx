...
import { getDailyStudyInsight, getInsightQuestionSource, legacyF108Sources, studyAnalysis } from "./studyAnalysis";
...
function sortStudyTasks(tasks: StudyTask[], subjects: Subject[], today: string): StudyTask[] {
  const subjectByCode = new Map(subjects.map((subject) => [subject.code, subject]));
  return [...tasks].sort((a, b) => {
    const aSubject = subjectByCode.get(a.subjectCode);
    const bSubject = subjectByCode.get(b.subjectCode);
    const aPriority = aSubject ? taskPriority(a, aSubject, today) : a.priority;
    const bPriority = bSubject ? taskPriority(b, bSubject, today) : b.priority;
    return bPriority - aPriority || a.dueDate.localeCompare(b.dueDate) || a.title.localeCompare(b.title);
  });
}

function updateTaskProgressInState(current: AppState, taskId: string, completionPercent: TaskCompletionPercent, today: string, toggleDone = false): AppState {
  const target = current.tasks.find((task) => task.id === taskId);
  if (!target) return current;
  const isUndo = toggleDone && target.status === "done";
  const nextStatus: StudyTask["status"] = isUndo ? "todo" : completionPercent === 100 ? "done" : "in-progress";
  const revisitDays = target.phase === "understand" ? 2 : target.phase === "retrieve" ? 4 : 7;
  const tasks = current.tasks.map((task) => task.id === taskId
    ? { ...task, status: nextStatus, completionPercent: isUndo ? undefined : completionPercent, lastCompletedAt: nextStatus === "done" ? today : task.lastCompletedAt, revisitDate: nextStatus === "done" ? addDays(today, revisitDays) : undefined }
    : task);
  if (nextStatus !== "done" || !target.chapterRange) return { ...current, tasks };
  const chapterNumbers = new Set(chapterNumbersFromRange(target.chapterRange));
  const chapters = current.chapters.map((chapter) => chapter.subjectCode === target.subjectCode && chapterNumbers.has(chapter.chapterNumber)
    ? { ...chapter, readThrough: true, updatedAt: new Date().toISOString() }
    : chapter);
  const currentChapter = chapters.filter((chapter) => chapter.subjectCode === target.subjectCode && chapter.readThrough).length;
  return { ...current, tasks, chapters, subjects: current.subjects.map((subject) => subject.code === target.subjectCode ? { ...subject, currentChapter } : subject) };
}
...
function App() {
  ...
  const setTaskProgress = useCallback((taskId: string, completionPercent: TaskCompletionPercent) => {
    updateState((current) => updateTaskProgressInState(current, taskId, completionPercent, today));
  ...
  const completeTask = useCallback((taskId: string) => {
    updateState((current) => updateTaskProgressInState(current, taskId, 100, today, true));
    setToast("Task updated. Honest progress counts.");
  }, [today, updateState]);

  const snoozeTask = useCallback((taskId: string) => {
    updateState((current) => ({ ...current, tasks: current.tasks.map((task) => task.id === taskId && !task.fixed ? { ...task, dueDate: addDays(task.dueDate < today ? today : task.dueDate, 1), status: "snoozed", manuallyScheduled: true } : task) }));
  ...
  return <StudyStateContext.Provider value={state}><div className="app-shell">
    ...
    <main className="main-content">
      ...
      <CelebrationBanner />
      {view === "dashboard" && <Dashboard state={state} today={today} setView={setView} completeTask={completeTask} setTaskProgress={setTaskProgress} snoozeTask={snoozeTask} revisitTask={revisitTask} reviewTask={reviewTask} />}
      {view === "plan" && <StudyPlan state={state} today={today} completeTask={completeTask} setTaskProgress={setTaskProgress} snoozeTask={snoozeTask} logSession={logSession} />}
...
function CelebrationBanner() {
  return <aside className="celebration-banner" aria-label="Exam celebration"><div className="celebration-icon">✦</div><div><strong>Congratulations, you don&apos;t have to write A311!</strong><p>One less exam-shaped cloud on the horizon. Your study plan just got a little lighter.</p></div><span>tiny victory</span></aside>;
}

function Dashboard({ state, today, setView, completeTask, setTaskProgress, snoozeTask, revisitTask, reviewTask }: { state: AppState; today: string; setView: (view: View) => void; completeTask: (id: string) => void; setTaskProgress: (id: string, percent: TaskCompletionPercent) => void; snoozeTask: (id: string) => void; revisitTask: (id: string) => void; reviewTask: (id: string, confidence: Confidence) => void }) {
  const pending = state.tasks.filter((task) => task.status !== "done" && !task.archived);
  const topTasks = sortStudyTasks(pending, state.subjects, today).slice(0, 3);
...
function ScheduleStatusCard({ summaries }: { summaries: PlannerSummary[] }) {
  ...
  return <section className={`schedule-status-card ${overallStatus}`} aria-label="Study schedule status"><div className="schedule-status-head"><div><div className="card-kicker"><span>RUNWAY STATUS</span><Icon name={overallStatus === "behind" ? "info" : "target"} size={15} /></div><h3>{heading}</h3></div><span className="schedule-status-pill">{overallStatus === "behind" ? "Behind" : overallStatus === "ahead" ? "Ahead" : "On track"}</span></div><div className="schedule-status-list">{summaries.map((summary) => <div className={`schedule-status-row ${summary.status}`} key={summary.subjectCode}><strong>{summary.subjectCode}</strong><span>{summary.status === "behind" ? `${summary.gap} chapters behind · about ${summary.weeksBehind} weeks` : summary.status === "ahead" ? `${summary.aheadBy} chapters ahead` : "On track with lectures"}</span></div>)}</div><p className="schedule-status-help">{behind.length > 0 ? "You are behind the lecture runway. Keep using the individual task checkboxes to make the next honest step visible; guided rebalancing will come later." : "Start with Today&apos;s first block and keep the runway steady. Guided rebalancing will come later."}</p></section>;
}

function AssessmentRunway({ events, today }: { events: CalendarEvent[]; today: string }) {
  const assessments = events
    .filter((event) => (event.kind === "test" || event.kind === "exam" || event.kind === "assignment") && event.dateConfirmed !== false && event.date >= today)
    .sort((a, b) => a.date.localeCompare(b.date));
  return <section className="assessment-runway section-block"><div className="section-heading compact"><div><p className="eyebrow">Dates and scope</p><h2>Assessment runway</h2></div><span className="soft-tag">lecture first</span></div><p className="muted">Use the calendar dates as anchors: prepare for the next lecture first, then use the scoped assessment blocks and past-paper runway.</p><div className="assessment-list">{assessments.map((event) => <article className={`assessment-item ${event.kind}`} key={event.id}><div><strong>{event.subjectCode} · {event.title}</strong><span>{formatLongDate(event.date)} · {event.durationMinutes ? `${event.durationMinutes / 60}-hour sitting` : ""}</span></div><p>{event.chapterRange ? `Scope: ${event.chapterRange}` : event.detail}</p></article>)}</div></section>;
}
...
function StudyPlan({ state, today, completeTask, setTaskProgress, snoozeTask, logSession }: { state: AppState; today: string; completeTask: (id: string) => void; setTaskProgress: (id: string, percent: TaskCompletionPercent) => void; snoozeTask: (id: string) => void; logSession: (draft: StudySessionDraft) => void }) {
  const [horizon, setHorizon] = useState<PlanBucket>("today");
  const [filter, setFilter] = useState<SubjectCode | "all">("all");
  const pending = sortStudyTasks(state.tasks.filter((task) => task.status !== "done" && !task.archived && (filter === "all" || task.subjectCode === filter)), state.subjects, today);
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">A plan with breathing room</p><h1>Study plan</h1><p className="lede">Start with {counts.today} small item{counts.today === 1 ? "" : "s"} today. The rest can wait its turn.</p></div></section><div className="plan-banner"><div className="plan-banner-icon"><Icon name="spark" size={20} /></div><div><strong>{overdueCount > 0 ? `${overdueCount} old blocks are still visible for an honest next step.` : "Your plan is ready for a good day."}</strong><p>{state.settings.lastReplannedAt ? `Last balanced ${formatShortDate(state.settings.lastReplannedAt.slice(0, 10))}.` : "Use the individual checkboxes and partial-progress buttons to keep the plan truthful. Guided rebalancing will come later."}</p></div><span className="plan-banner-tag">editable</span></div><div className="plan-guidance"><span className="guidance-number">01</span><div><strong>Priority order: lecture readiness → tests and exams → past papers.</strong><p>Catch-up supports the assessment runway, but the next lecture comes first so the gap does not keep growing. Older papers become short question drills; the newest papers stay as full three-hour sittings.</p></div></div><div className="plan-guidance"><span className="guidance-number">02</span><div><strong>Pick the first block, not the perfect day.</strong><p>Choose 50% or 80% when that is the honest finish. Partial work stays visible and can be polished later.</p></div></div><div className="plan-horizon-row" aria-label="Plan horizon">{(["today", "tomorrow", "week", "later", "all"] as const).map((item) => <button key={item} className={`horizon-pill ${horizon === item ? "selected" : ""}`} onClick={() => setHorizon(item)}><strong>{bucketLabels[item]}</strong><span>{counts[item]}</span></button>)}</div><div className="plan-filter-line"><span className="filter-label">Subject</span>{(["all", "F102", "F108"] as const).map((item) => <button key={item} className={`filter-pill ${filter === item ? "selected" : ""}`} onClick={() => setFilter(item)}>{item === "all" ? "All" : item}</button>)}</div><DailyExamLens tasks={pending} /><div className="plan-layout"><div>{horizon === "all" ? buckets.map((bucket) => <PlanBucketSection key={bucket} bucket={bucket} tasks={pending.filter((task) => getPlanBucket(task, today) === bucket)} today={today} completeTask={completeTask} setTaskProgress={setTaskProgress} snoozeTask={snoozeTask} />) : <PlanBucketSection bucket={horizon} tasks={visibleTasks} today={today} completeTask={completeTask} setTaskProgress={setTaskProgress} snoozeTask={snoozeTask} />}{visibleTasks.length === 0 && horizon !== "all" && <EmptyState title={`Nothing in ${bucketLabels[horizon].toLowerCase()}.`} detail="A quiet plan is a real plan too." />}</div><FocusTimer subjects={state.subjects} tasks={horizon === "all" ? pending : visibleTasks} logSession={logSession} /></div></div>;
}

function DailyExamLens({ tasks }: { tasks: StudyTask[] }) {
  const subjectCode = tasks[0]?.subjectCode ?? "F102";
  const insight = getDailyStudyInsight(subjectCode);
  return <section className={`daily-exam-lens ${subjectCode.toLowerCase()}`} aria-label="Daily exam lens"><div className="daily-exam-lens-head"><div><p className="eyebrow">Today&apos;s exam lens</p><h2>{subjectCode} <span>· {insight.scope}</span></h2></div><span className="lens-tag">source-backed priority</span></div><p className="daily-exam-focus"><strong>Focus:</strong> {insight.focus}</p><div className="daily-exam-columns"><div><span>Watch for</span><p>{insight.examinerTrap}</p></div><div><span>Try this</span><p>{insight.questionPrompts[0]}</p></div></div><a className="analysis-source" href={getInsightQuestionSource(insight, 0)} target="_blank" rel="noreferrer">{insight.sourceLabel} <Icon name="arrow" size={12} /></a><p className="lens-note">Curated priority signal, not an exhaustive frequency estimate. Use the source pack on Subjects for the full archive.</p></section>;
}
...
function CalendarView({ events, today }: { events: CalendarEvent[]; today: string }) {
  ...
  const visibleEvents = filtered.filter((event) => event.date >= weekDays[0] && event.date <= weekDays[6]);
  const selectedEvent = filtered.find((event) => event.id === selectedEventId) ?? null;

  useEffect(() => {
    setSelectedEventId((current) => current && visibleEvents.some((event) => event.id === current)
      ? current
      : visibleEvents[0]?.id ?? filtered.find((event) => event.date >= today)?.id ?? filtered[0]?.id ?? null);
  }, [filter, events, today, weekStartDate]);

  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Milestones without the overwhelm</p><h1>Calendar</h1><p className="lede">See every lecture, assessment, and exam in the same week view. Choose a day for the useful detail.</p></div><div className="calendar-key"><span><i className="key-dot blue" /> F102</span><span><i className="key-dot pink" /> F108</span><span><i className="key-dot neutral" /> shared</span></div></section><div className="filter-row calendar-filters"><span className="filter-label">Filter</span>{(["all", "F102", "F108"] as const).map((item) => <button key={item} className={`filter-pill ${filter === item ? "selected" : ""}`} aria-pressed={filter === item} onClick={() => setFilter(item)}>{item === "all" ? "All milestones" : item}</button>)}</div><section className="calendar-board"><div className="calendar-board-head"><div><p className="eyebrow">Week view</p><h2>{formatShortDate(weekDays[0])} – {formatShortDate(weekDays[6])}</h2></div><div className="calendar-nav"><button className="text-button" onClick={() => setWeekStartDate((current) => addDays(current, -7))} aria-label="Show previous week"><Icon name="arrow" size={15} /> Previous</button><button className="filter-pill" onClick={() => setWeekStartDate(startOfWeek(today))}>This week</button><button className="text-button next" onClick={() => setWeekStartDate((current) => addDays(current, 7))} aria-label="Show next week">Next <Icon name="arrow" size={15} /></button></div></div><div className="calendar-week-grid">{weekDays.map((date) => { const dayEvents = visibleEvents.filter((event) => event.date === date); const isToday = date === today; return <section className={`calendar-day ${isToday ? "today" : ""}`} key={date}><header className="calendar-day-head"><span>{new Intl.DateTimeFormat("en-ZA", { weekday: "short" }).format(new Date(`${date}T12:00:00`))}</span><strong>{new Intl.DateTimeFormat("en-ZA", { day: "numeric" }).format(new Date(`${date}T12:00:00`))}</strong>{isToday && <i>today</i>}</header><div className="calendar-day-events">{dayEvents.length > 0 ? dayEvents.map((event) => <CalendarEventCard key={event.id} event={event} today={today} selected={event.id === selectedEventId} onSelect={setSelectedEventId} />) : <p className="calendar-empty">Open space</p>}</div></section>; })}</div><div className="calendar-board-foot"><span>{visibleEvents.length} milestone{visibleEvents.length === 1 ? "" : "s"} this week</span><span>Lectures stay visible even when several subjects share a day.</span></div></section>{selectedEvent && <section className={`calendar-detail-card ${selectedEvent.subjectCode?.toLowerCase() ?? "neutral"}`}><div className="calendar-detail-date"><strong>{selectedEvent.dateConfirmed === false ? "TBC" : formatLongDate(selectedEvent.date)}</strong><span>{selectedEvent.subjectCode ?? "Shared milestone"}</span></div><div className="calendar-detail-body"><div className="event-topline"><span className="event-kind">{selectedEvent.kind}</span>{selectedEvent.durationMinutes && <span className="event-duration">{selectedEvent.durationMinutes / 60}h</span>}{selectedEvent.dateConfirmed === false && <span className="event-badge">date TBC</span>}</div><h3>{selectedEvent.title}</h3><p>{selectedEvent.chapterRange ? `${selectedEvent.chapterRange} · ` : ""}{selectedEvent.detail ?? "Keep this milestone visible in the wider plan."}</p>{selectedEvent.kind === "lecture" && <p className="calendar-detail-note">Use this event to finish the assigned reading before class, then carry one uncertainty into the lecture.</p>}</div>{selectedEvent.kind === "exam" && <span className="event-badge">exam day</span>}</section>}</div>;
}
...
function CalendarEventCard({ event, today, selected, onSelect }: { event: CalendarEvent; today: string; selected: boolean; onSelect: (id: string) => void }) {
  const isPast = event.dateConfirmed !== false && event.date < today;
  return <button className={`calendar-event-card ${event.subjectCode?.toLowerCase() ?? "neutral"} ${event.kind} ${isPast ? "past" : ""} ${selected ? "selected" : ""}`} onClick={() => onSelect(event.id)} aria-label={`Show details for ${event.subjectCode ? `${event.subjectCode} ` : ""}${event.title} on ${formatLongDate(event.date)}${event.chapterRange ? `, ${event.chapterRange}` : ""}`}><span className="calendar-event-kind">{event.kind}</span><strong>{event.subjectCode && `${event.subjectCode} · `}{event.title}</strong><span>{event.chapterRange ?? event.detail ?? "Milestone"}</span></button>;
}
...
function SubjectsView({ state, today, saveCheckpoint, logSession, toggleChapterCheck, setChapterCount }: { state: AppState; today: string; saveCheckpoint: (checkpoint: ChapterCheckpoint) => void; logSession: (draft: StudySessionDraft) => void; toggleChapterCheck: (subjectCode: SubjectCode, chapterNumber: number, check: ChapterCheck) => void; setChapterCount: (subjectCode: SubjectCode, total: number) => void }) {
  ...
  return <div className="page"><section className="page-intro"><div><p className="eyebrow">Know where you are</p><h1>Subjects</h1><p className="lede">Short checkpoints and exam-style records, instead of long summaries that become another textbook.</p></div><a className="button secondary-button" href="https://www.actuarialsociety.org.za/document-category/past-paper/" target="_blank" rel="noreferrer">ASSA past papers <Icon name="arrow" size={15} /> </a></section><div className="subject-tabs">{state.subjects.map((item) => <button key={item.code} className={`subject-tab ${active === item.code ? "active" : ""} ${item.color}`} onClick={() => setActive(item.code)}><span className="subject-tab-dot" /><strong>{item.code}</strong><small>{item.shortName}</small></button>)}</div><section className={`subject-hero ${subject.color}`}><div><div className="hero-code">{subject.code}</div><h2>{subject.name}</h2><p>{subject.description}</p><div className="hero-exam"><Icon name="calendar" size={15} /> Exam {subject.examDates.map(formatShortDate).join(" and ")} · {daysUntil(subject.examDates[0], today)} days</div></div><div className="hero-progress"><div className="progress-circle"><span>{progress.coveragePercent}<small>%</small></span></div><span>{progress.label}</span></div></section><div className="metric-strip"><div><strong>{hasChapters ? `${chapterMetrics.readThrough}/${chapterMetrics.total}` : "0"}</strong><span>read through</span></div><div><strong>{hasChapters ? `${chapterMetrics.summarized}/${chapterMetrics.total}` : "0"}</strong><span>short summaries</span></div><div><strong>{hasChapters ? `${chapterMetrics.confident}/${chapterMetrics.total}` : "0"}</strong><span>confidence</span></div></div><section className="course-notes-card detail-card"><div className="section-heading compact"><div><p className="eyebrow">Course notes</p><h3>{subject.syllabusChapterTotal} chapters + reference sections</h3></div><span className="soft-tag">{subject.supplementalSections.length} reference</span></div><p className="muted">Track the examinable chapters separately, then keep the glossary and acronym pages close for definitions and terminology.</p><div className="course-note-tags"><span>{subject.syllabusChapterTotal} examinable chapters</span>{subject.supplementalSections.map((section) => <span key={section}>{section}</span>)}</div></section><StudyAnalysisPanel subjectCode={active} /><ChapterTracker subject={subject} chapters={state.chapters} onToggle={toggleChapterCheck} onSetTotal={setChapterCount} /><div className="subject-detail-grid"><section className="detail-card"><div className="section-heading compact"><div><p className="eyebrow">A small summary, on purpose</p><h3>Chapter checkpoint</h3></div><span className="soft-tag">5 minutes</span></div><p className="muted">Keep this short enough to trust. It is for returning to, not retyping the chapter.</p><CheckpointForm key={checkpoint.id} checkpoint={checkpoint} onSave={saveCheckpoint} /></section><section className="detail-card"><div className="section-heading compact"><div><p className="eyebrow">Practice is the proof</p><h3>Past-paper record</h3></div><Icon name="target" size={18} /></div><p className="muted">Log the attempt, score if you have one, the errors that matter, and the next action. This creates feedback instead of just more hours.</p><PaperLogForm key={active} subjectCode={active} today={today} onSave={logSession} /><a className="resource-link" href="https://www.actuarialsociety.org.za/exams/" target="_blank" rel="noreferrer"><span>ASSA exam information</span><Icon name="arrow" size={14} /></a></section></div>{papers.length > 0 && <section className="paper-history detail-card"><div className="section-heading compact"><div><p className="eyebrow">Evidence you can use</p><h3>Recent paper records</h3></div><span className="soft-tag">{papers.length} logged</span></div>{papers.map((paper) => <div className="paper-row" key={paper.id}><div><strong>{paper.paperName || "Past paper"}</strong><span>{paper.paperDate ? formatShortDate(paper.paperDate) : formatShortDate(paper.date)} · {paper.attemptedMinutes ?? paper.durationMinutes} min</span></div><div className="paper-stats"><b>{paper.score === undefined ? "—" : `${paper.score}%`}</b><span>{paper.errorCount === undefined ? "No error count" : `${paper.errorCount} errors`}</span></div></div>)}</section>}</div>;
}

function StudyAnalysisPanel({ subjectCode }: { subjectCode: SubjectCode }) {
  const analysis = studyAnalysis[subjectCode];
  return <section className={`study-analysis-panel ${subjectCode.toLowerCase()}`}><div className="study-analysis-head"><div><p className="eyebrow">Past-paper signals</p><h3>{analysis.title}</h3><p>{analysis.summary}</p></div><a className="resource-link" href={analysis.sourceUrl} target="_blank" rel="noreferrer"><span>Open ASSA index</span><Icon name="arrow" size={14} /></a></div><p className="study-analysis-note">{analysis.sourceNote}</p><div className="paper-strategy"><div><p className="eyebrow">All available papers</p><h4>{analysis.paperStrategy.title}</h4><p>{analysis.paperStrategy.detail}</p></div><ol>{analysis.paperStrategy.steps.map((step) => <li key={step}>{step}</li>)}</ol></div><div className="analysis-grid">{analysis.insights.map((insight) => <article className="analysis-card" key={insight.id}><div className="analysis-card-top"><span>{insight.scope}</span><b className={insight.priority}>{insight.priority === "high" ? "high signal" : insight.priority}</b></div><strong>{insight.focus}</strong><p><span>Examiner trap:</span> {insight.examinerTrap}</p><div className="analysis-questions"><span>Try:</span>{insight.questionPrompts.map((prompt, index) => <a key={prompt} href={getInsightQuestionSource(insight, index)} target="_blank" rel="noreferrer">{prompt}</a>)}</div><a className="analysis-source" href={insight.sourceUrl} target="_blank" rel="noreferrer">{insight.sourceLabel} <Icon name="arrow" size={12} /></a></article>)}</div><SourcePack links={analysis.sourceLinks} />{subjectCode === "F108" && <div className="legacy-source-row"><span>F108 also carries forward:</span>{legacyF108Sources.map((source) => <a key={source.label} href={source.url} target="_blank" rel="noreferrer">{source.label}</a>)}</div>}</section>;
}

function SourcePack({ links }: { links: Array<{ label: string; url: string }> }) {
  return <div className="analysis-source-pack"><span>Official source pack</span><div>{links.map((link) => <a key={link.url} href={link.url} target="_blank" rel="noreferrer">{link.label} <Icon name="arrow" size={11} /></a>)}</div></div>;
}
