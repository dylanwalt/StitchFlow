...
import { getDailyStudyInsight, legacyF108Sources, studyAnalysis } from "./studyAnalysis";
...
function ConfirmationPanel() {
  return <section className="confirmation-panel" aria-label="Details needing confirmation"><div className="card-kicker"><span>NEEDS YOUR INPUT</span><Icon name="info" size={15} /></div><h3>Confirm the remaining details</h3><p>These are the only items I have left open rather than guessing.</p><ol><li>Which subject is the missing Thursday lecture for, and what exact date and time should be added?</li><li>Can you upload or export the Course Notes locally, or provide page counts by chapter? No email access or sending is needed.</li><li>Should the ASSA analysis cover every available paper, or a specific year window for each subject?</li><li>Do your course-note chapter numbers match the current F102 1-36 and F108 1-23 tracker?</li></ol></section>;
}
...
function CalendarEventCard({ event, today, selected, onSelect }: { event: CalendarEvent; today: string; selected: boolean; onSelect: (id: string) => void }) {
  ...
  return <button className={`calendar-event-card ${event.subjectCode?.toLowerCase() ?? "neutral"} ${event.kind} ${isPast ? "past" : ""} ${selected ? "selected" : ""}`} onClick={() => onSelect(event.id)} aria-label={`Show details for ${event.title}`}><span className="calendar-event-kind">{event.kind}</span><strong>{event.subjectCode && `${event.subjectCode} · `}{event.title}</strong><span>{event.chapterRange ?? event.detail ?? "Milestone"}</span></button>;
}
